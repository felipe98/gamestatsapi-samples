...
function animate() {
  if (!stop) {
    ...
    if (apihelper.setFrameReady() == true) {
        setFrame(apihelper, canvas.toDataURL('image/png'));
    }
    if (KEY_STATUS.capture) {
        var snap = new Snapshot();
        snap.Data = canvas.toDataURL('image/png');
        snap.IsPublic = true;
        snap.Platform = 'Html 5';
        snap.PlayerId = 'felipe.ramos@unlimited-software.com';
        snap.ProjectId = 1;

        api.persistSnapshot(snap,
            function (data) { });
    }
...
function gameOver() {
  ...
  assetLoader.sounds.gameOver.play();
    ...
    // Capture last frame for sequence
  if (apihelper.setFrameReady() == true) {
      setFrame(apihelper, canvas.toDataURL('image/png'));
  }
  apihelper.meta = JSON.stringify({ "Score": score });
  api.persistSequence(apihelper.buildSequenceJson(), function (data) { });
}
